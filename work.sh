...
CYGWIN=binmode
export CYGWIN
export LIB66_SCRIPT_EDIT_DATE="2025-01-30"
export LIB66_LIBRARY_API_VERSION="0.1"
export LIB66_LIBRARY_VERSION="$LIB66_LIBRARY_API_VERSION.0"
export LIB66="lib66"
export LIB66_LIBRARY="$LIB66-$LIB66_LIBRARY_API_VERSION"
...
if test "$DOVERSION" = "yes" ; then
   echo "Lib66 version $LIB66_LIBRARY_VERSION $LIB66_SCRIPT_EDIT_DATE"
   exit 0
fi
...
if test "$DOHELP" = "yes" ; then
   ...
   cat << E_O_F
Usage: ./work [options]    ($LIB66_LIBRARY_VERSION-$LIB66_SCRIPT_EDIT_DATE)
...
be more to come. Some options might not work on Windows.
 ...
 --clean             Delete the usual derived files from the project. Also
                     do "git checkout doc/lib66-library-guide.pdf"
...
#  rm -f doc/latex/*.log
#  echo "Build products removed from the lib66/build directory."
#  git checkout doc/lib66-library-guide.pdf tests/data/fooout.rc
...
# https://mesonbuild.com/Running-Meson.html
   ...
   echo "Making all lib66 libraries..."
...
if test "$DOINSTALL" = "yes" ; then
   ...
   if test "$USERID" = 0 ; then
      ...
      echo "UID $USERID. We want you as root to install the lib66 library..."
   fi
...
if test "$DOUNINSTALL" = "yes" ; then
   ...
   USERID=$(id -u)
   if test "$USERID" = 0 ; then
      echo "Uninstalling the lib66 library..."
      ninja -C build uninstall
      if test "$PLATFORM" = "UNIX" ; then
         rm -rf "$INSTALL_PREFIX/include/$LIB66_LIBRARY"
         rm -rf "$INSTALL_PREFIX/$INSTALL_LIBDIR/$LIB66_LIBRARY"
         rm -rf "$INSTALL_PREFIX/share/doc/$LIB66"
#        rm -rf "$INSTALL_PREFIX/man/man1/$LIB66.1"
      fi
   else
      echo "UID $USERID. We want you as root to uninstall the lib66 library..."
   fi
